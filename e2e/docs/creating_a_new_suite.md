...
- The backend on `http://localhost:8086`, seeded with `seed.sql` only:
  Alice ( `bookworm` ) and Bob ( `pageturner` ), three cooks, six recipes,
  five shopping items.
- The front end on `http://localhost:5173` ( `baseURL`, so
  `page.goto("/recipes")` works ). ONE front end serves every suite - it is
...
- Emails are asynchronous. Always clear, act, then poll:
  ...
  const message = await waitForMessageTo(email, { subjectContains: "Welcome" });
  expect(message.Content.Body).toContain(name);
  ```
